...
function formatDuration(seconds) {
  ...
  const numDays = Math.floor(tmpSeconds / NUM_SEC_PER_DAY);
  tmpSeconds -= numDays * NUM_SEC_PER_DAY;
  const numHrs = Math.floor(tmpSeconds / NUM_SEC_PER_HR);
  tmpSeconds -= numHrs * NUM_SEC_PER_HR;
  const numMins = Math.floor(tmpSeconds / NUM_SEC_PER_MIN);
  const numSecs = tmpSeconds - numMins * NUM_SEC_PER_MIN;
